...
def signup():
    ...
    if request.method == 'GET':
        ...
        if redup_data:
            ...
            sql = "INSERT INTO user_table(userid, password, sub_date) VALUES('%s', '%s', '%s')" % (
            userid, password, subd)
            cursor.execute(sql)
            data = cursor.fetchall()
...
def profile():
    ...
    if count_user_picture[0] == 0:
        return render_template('/profile.html', timedata=timedata[0], count_all_picture=count_all_picture[0],
                               count_user_picture=count_user_picture[0], percent=0, tup_face=tup_face,
                               tup_name=tup_name)
    else:
        percent = (count_user_picture[0] / count_all_picture[0]) * 100
        percent = '%0.1f' % percent
        return render_template('/profile.html', timedata=timedata[0], count_all_picture=count_all_picture[0],
                               count_user_picture=count_user_picture[0], percent=percent, tup_name=tup_name,
                               tup_face=tup_face)
    ...
    if request.method == 'GET':
        ...
        if not data:
            ...
            return render_template('/face.html', img=image, msg=msg, face_detect=face_detect, face_list=face_list,
                                   face_rec=face_rec, face_content=face_content)
...
def picture():
    ...
    cursor.close()
    conn.close()
    return render_template('/picture.html', seeall=seeall, num=num, get_image=get_image, get_content=get_content,
                           get_image_all=get_image_all, get_content_all=get_content_all, imagelen=len(get_content_all),
                           get_date_all=get_date_all, get_subid_all=get_subid_all)
